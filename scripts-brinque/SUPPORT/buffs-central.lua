-- =========================================================
-- BUFFS (botao + painel EDIT + macro recast por duracao)
...
if not root then
  print("[BUFFS] root nao encontrado"); return
end

-- -------------------------
-- Defaults persistentes
-- -------------------------
storage.buffsEnabled = (storage.buffsEnabled ~= false)

-- Cada buff:
-- { on=true/false, name="texto", spell="fala", duration=180, cooldown=1000, minMana=0, safe=1500, last=0 }
if type(storage.buffList) ~= "table" then
  storage.buffList = {
    { on=true, name="Utamo", spell="utamo vita", duration=180, cooldown=1000, minMana=0, safe=2000, last=0 },
  }
end
...
local function refreshDelAllState()
  ...
  local empty = (#storage.buffList == 0)
...
-- monta/atualiza um card de buff (linha)
local function mountBuffCard(card, idx, rebuildEditBody)
  local b = storage.buffList[idx]; if not b then return end
...
MainWindow
  ...
  Button
    ...
    mkLabel("duration(s):");           local durEdit   = mkEdit("durEdit")
    ...
    durEdit:setText(tostring(num(b.duration or 0,0)))
    cdEdit:setText(tostring(num(b.cooldown or 0,0)))
    ...
    saveB.onClick = function()
      ...
      b.duration = num(durEdit:getText(), 0)
      b.cooldown = num(cdEdit:getText(), 0)
  ...
  local btnDel = UI.Button("DEL", function()
    table.remove(storage.buffList, idx)
    rebuildEditBody()
    refreshDelAllState()
  end)
  btnDel:setWidth(52)
...
-- REBUILD: 2 cards por linha com zebragem
local function rebuildEditBody()
  if not editUI then return end
  local body = editUI:getChildById("body"); if not body then return end
  body:destroyChildren()

  local n = #storage.buffList
  if n == 0 then
    local empty = UI.Label("Lista vazia — clique em 'NOVO BUFF'")
...
Panel
  layout:
    ...
    cardA:setWidth(170)
    mountBuffCard(cardA, i, rebuildEditBody)
...
Panel
  layout:
    ...
    fit-children: true
      ...
      mountBuffCard(cardB, i + 1, rebuildEditBody)
    end
...
-- Abre/fecha painel EDIT
local function toggleEditPanel()
  local old = root:getChildById("editPanelBuff")
  if old then old:destroy(); editUI = nil; return end

  editUI = setupUI([[
MainWindow
  id: editPanelBuff
  text: "CENTRAL DE BUFFS"
  size: 380 380
  color: #f7ff12
  background-color: black
  opacity: 0.92
  ...
  editUI:getChildById("addButton").onClick = function()
    table.insert(storage.buffList, { on=true, name="Buff", spell="", duration=60, cooldown=1000, minMana=0, safe=1500, last=0 })
...
-- -------------------------
if btnBuffsBrq and not btnBuffsBrq:isDestroyed() then btnBuffsBrq:destroy() end
if btnBuffsEditBrq and not btnBuffsEditBrq:isDestroyed() then btnBuffsEditBrq:destroy() end

btnBuffsBrq = UI.Button("(BUFFS)", function()
  storage.buffsEnabled = not storage.buffsEnabled
  btnBuffsBrq:setText("(BUFFS "..(storage.buffsEnabled and "ON" or "OFF")..")")
  btnBuffsBrq:setColor(storage.buffsEnabled and "#21fff8" or "#666666")
end)
btnBuffsBrq:setWidth(100)
btnBuffsBrq:setStyle("margin-right: 6px;")
btnBuffsBrq:setText("(BUFFS "..(storage.buffsEnabled and "ON" or "OFF")..")")
btnBuffsBrq:setColor(storage.buffsEnabled and "#21fff8" or "#666666")

btnBuffsEditBrq = UI.Button("(EDITAR BUFFS)", toggleEditPanel)
btnBuffsEditBrq:setWidth(120)
btnBuffsEditBrq:setColor("#ecff21")

-- -------------------------
-- MACRO: recast por duracao/cooldown
-- -------------------------
local lastCastAt = 0
macro(200, function()
  if not storage.buffsEnabled then return end
  ...
  for _, b in ipairs(storage.buffList) do
    if b.on and (b.spell or "") ~= "" then
      local needMana = num(b.minMana, 0)
      if needMana == 0 or mp >= needMana then
        local dur  = num(b.duration, 0)
        local cd   = num(b.cooldown, 0)
        local safe = num(b.safe, 1500)
        b.last     = num(b.last, 0)

        local expired = (dur == 0) or (tnow - b.last >= math.max(0, dur*1000 - safe))
        local cdReady = (tnow - lastCastAt >= 400) and (tnow - b.last >= cd)

        if expired and cdReady then
          say(b.spell)
          b.last = tnow
          lastCastAt = tnow
          break -- um por tick para evitar flood
        end
      end
    end
  end
